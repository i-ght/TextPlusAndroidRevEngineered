...
namespace LibTextPlus.ContactDataRetriever
{
    public class Cfg
    {
        ...
        public int MinReqPerSession { get; }
        public int MaxReqPerSession { get; }
        public int DelayInMinBetweenSessions { get; }
        public int MinReqDelayInSeconds { get; }
        public int MaxReqDelayInSeconds { get; }

        public Cfg(
            int maxWorkers,
            int minReqPerSession,
            int maxReqPerSession,
            int delayInMinBetweenSessions,
            int minReqDelayInSeconds,
            int maxReqDelayInSeconds)
        {
            MaxWorkers = maxWorkers;
            MinReqPerSession = minReqPerSession;
            MaxReqPerSession = maxReqPerSession;
            DelayInMinBetweenSessions = delayInMinBetweenSessions;
            MinReqDelayInSeconds = minReqDelayInSeconds;
            MaxReqDelayInSeconds = maxReqDelayInSeconds;
        }
    }
...
#pragma warning disable CS8618 // Non-nullable field is uninitialized.
        ...
        public Queue<TextPlusMatchableSearchResponse> Contacts { get; set; }
...
#pragma warning restore CS8618 // Non-nullable field is uninitialized.
    ...
    public class AgentConfig
    {
        ...
        public Cfg Cfg { get; }
        public AuthTextPlusSession Session { get; }
        public Queue<TextPlusMatchableSearchResponse> Contacts { get; }
        ...
        public AgentConfig(
            ...
            Cfg cfg,
            AuthTextPlusSession session,
            Queue<TextPlusMatchableSearchResponse> contacts,
    ...
    public class Agent : IDisposable
    {
        ...
        {
            var seconds = RandomModule.Next(_state.Cfg.MinReqDelayInSeconds, _state.Cfg.MaxReqDelayInSeconds);
                ...
                try
                {
                    ...
                    while (_state.Contacts.Count > 0)
                    {
                        try {
                            ...
                            using var jidResp = await TextPlusApiClient.FindPersonaByJid(_props.Session, contact.Persona.Jid);
                            if (jidResp.Value.Embedded.Personae.Count == 0)
                                continue;
                            using var resp = await TextPlusApiClient.RetrieveUserInfoDeetz(_props.Session, jidResp.Value.Embedded.Personae[0].User.Id).ConfigureAwait(false);
                            var gender = resp.Value.primaryPersona.sex;
                            var txt = JsonConvert.SerializeObject(resp.Value, Formatting.None);
                            if (gender == null) {
                                _props.UnkOutput.Add(txt);
                            }
                            else if (gender.InvariantStartsWith("m")) {
                                _props.MaleOutput.Add(txt);
                            }
                            else if (gender.InvariantStartsWith("f")) {
                                _props.FemaleOutput.Add(txt);
                            }
                            else {
                                _props.UnkOutput.Add(txt);
                            }
                            _state.ContactBlacklist.ThreadSafeAdd(contact.Persona.Handle);
                            ttl++;
                            _props.StatsUIUpdater.IncrementRetrieved(1);
                        } catch (TextPlusApiException e) when (e.HttpCtx.HttpResp.StatusCode == HttpStatusCode.NotFound) {
                    ...
                    if (ttl > 0)
                    {
                        ...
                        UpdateStatus($"retrieved {ttl} {word}.");
...
#pragma warning restore CS8618 // Non-nullable field is uninitialized.
    ...
    public class Director
    {
        ...
        private static Option<TextPlusMatchableSearchResponse> TryDeserializeUserInfo(string txt)
        {
            try
            {
                var result = JsonConvert.DeserializeObject<TextPlusMatchableSearchResponse>(txt);
                if (result == null
                    || string.IsNullOrWhiteSpace(result.Id)
                    || result.Persona == null
                    || string.IsNullOrWhiteSpace(result.Persona.Jid))
                {
                    return Option.None;
                }
        ...
        private Queue<TextPlusMatchableSearchResponse> GetContacts()
        {
            var cfg = _state.Cfg;
            var cnt = RandomModule.Next(cfg.MinReqPerSession, cfg.MaxReqPerSession);
            var contacts = _state.Seqs.Contacts;
            var queue = new Queue<TextPlusMatchableSearchResponse>(cnt);
            while (_state.ContactCache.Count > 0 && queue.Count < cnt)
            {
                var contact = _state.ContactCache.Dequeue();
                var contactInfo = TryDeserializeUserInfo(contact);
                if (contactInfo.IsSome && !_props.ContactBlacklist.Contains(contactInfo.Value.Persona.Handle))
                    queue.Enqueue(contactInfo.Value);
            }
        ...
        private bool ShouldSearchWithSession(AuthTextPlusSession session)
        {
            ...
            var added = sessionEndedAt + TimeSpan.FromMinutes(_state.Cfg.DelayInMinBetweenSessions);
        ...
        private Unit ActivateAgent(ActivateAgentDirective d)
        {
            ...
            if (!ShouldSearchWithSession(session))
            {
                ...
                UpdateAgentUI(d.Index, session.Username, $"this session is scheduled to search at {_state.SessionTimes[session.UserId].AddMinutes(_state.Cfg.DelayInMinBetweenSessions)}");
